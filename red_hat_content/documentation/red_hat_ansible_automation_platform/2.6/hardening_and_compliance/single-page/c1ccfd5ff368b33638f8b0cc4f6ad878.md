+++
path = "/documentation/en-us/red_hat_ansible_automation_platform/2.6/html-single/hardening_and_compliance/index"
title = "Hardening and compliance - Red Hat Ansible Automation Platform 2.6"
...
html = "data/docs_assets_pv1/red_hat_ansible_automation_platform/2.6/hardening_and_compliance/single-page/c1ccfd5ff368b33638f8b0cc4f6ad878.html"
last_crumb = "Hardening and compliance"
modified = "2026-03-02T19:58:11.000Z"
multi_page_path = "/documentation/en-us/red_hat_ansible_automation_platform/2.6/html/hardening_and_compliance/index/"
...
product_version = "2.6"
reference_url = "https://docs.redhat.com/en/documentation/red_hat_ansible_automation_platform/2.6/html-single/hardening_and_compliance/index"
solr_index = "true"
toc = "data/docs_assets_pv1/red_hat_ansible_automation_platform/2.6/hardening_and_compliance/toc/edb050bd8e2769132f71e108e468edfb.json"
type = "single-page"
+++
...
[Legal Notice](#idm140601132926496)
...
When installing Ansible Automation Platform, the installation program script checks that certain infrastructure servers are defined with a *Fully Qualified Domain Name* (FQDN) in the installation program inventory. All Ansible Automation Platform infrastructure nodes must have a valid FQDN defined in DNS which resolves to a routable IP address. These FQDNs are used in the installation program inventory file.

#### 2.1.2.2. DNS and load balancing

When using a load balancer with Ansible Automation Platform as described in the enterprise deployment topology, an additional FQDN is needed for the load-balancer. For example, an FQDN such as `aap.example.com` might be used for the load balancer which in turn directs traffic to each of the platform gateway components defined in the installation inventory.

#### 2.1.2.3. NTP

Configure each server in the Ansible Automation Platform infrastructure to synchronize time with an *Network Time Protocol* (NTP) pool or your organization’s NTP service. This ensures that logging and auditing events generated by Ansible Automation Platform have an accurate timestamp, and that any scheduled automation jobs execute at the correct time. This also enables communication between the systems within Ansible Automation Platform to not reject messages based on timeouts.
...
Guidance is provided for *Red Hat Enterprise Linux* (RHEL) managed nodes, but the concepts might be applicable to other Linux distributions as well.

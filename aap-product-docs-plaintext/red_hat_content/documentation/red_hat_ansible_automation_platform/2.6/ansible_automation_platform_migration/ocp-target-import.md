...
1. Wait for component pods to stop. Only the 6 Operator pods will remain running.
...
1. Prepare a temporary environment for the database restore.
...
1. Test the environment variable:
...
1. Copy the artifact and checksum to the PVC:
...
1. Restore databases to Ansible Automation Platform PostgreSQL by using the temporary PostgreSQL pod.
...
1. Enter into the temporary PostgreSQL deployment and change directory to the mounted PVC containing the copied artifact:
...
1. Inside the pod, change directory to `        /tmp/aap-temp-pvc` and list its contents:
...
1. Extract the artifact and verify its contents:
...
1. Drop the automation controller database:
...
1. Alter the user temporarily with the `        CREATEDB` role:
...
1. Revert temporary user permission:
...
1. Restore the automation controller database:
...
1. Restore the automation hub database:
...
1. Restore the platform gateway database:
...
1. Replace database field encryption secrets and clean up temporary resources.
...
1. Scale Ansible Automation Platform components back up.
...
1. Enter into the automation controller pod:
...
1. Remove old nodes with `        awx-manage` , leaving only `        aap-controller-task` :

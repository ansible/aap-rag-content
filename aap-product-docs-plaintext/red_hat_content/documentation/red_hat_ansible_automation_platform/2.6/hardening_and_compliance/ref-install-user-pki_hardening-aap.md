...
## 2.2. Installation
### 2.2.3. Install with user-provided PKI certificates

By default, Ansible Automation Platform creates self-signed *Public Key Infrastructure* (PKI) certificates for the infrastructure components of the platform.

Where an existing PKI infrastructure is available, generate certificates for the automation controller, private automation hub, Event-Driven Ansible controller, and the PostgreSQL database server.
...
| <br> `postgres_ssl_key` | <br> `postgresql_tls_key` | <br>  The file name of the database server PKI key located in the installation program directory. This variable is only needed for the installer-managed database server, not if a third-party database is used. |

...
2. Create an Ansible vault file by using the following command:

`ansible-vault create vault.yml`

3. When prompted, enter a password. This password is required to access or modify the Ansible vault file and is required for day-two operations such as backups and reconfigurations.


Important
Passwords with special characters must be in double quotes.

4. Add your sensitive variables to the Ansible vault file and ensure they are not also defined in the installation program inventory file.

5. Store the vault password securely in accordance with your organization’s security policy, for example, using a password manager or a service like Hashicorp Vault.

...
- For RPM installations: Event-Driven Ansible must be installed on a separate server and cannot be installed on the same host as automation hub and automation controller.
- For containerized installations: Event-Driven Ansible is co-located with all other Ansible Automation Platform components in the growth topolgy. In the enterprise topology, Event-Driven Ansible runs on separate dedicated nodes.

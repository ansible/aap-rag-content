...
| Value | Description |
| --- | --- |
| <br> `ANSIBLE_GALAXY_CLI_COLLECTION_OPTS` | <br>  Passes arbitrary arguments to the ansible-galaxy CLI during the collection installation phase. <br>  For example, the `-pre` flag to enable the installation of pre-release collections, or `-c` to disable verification of the server’s SSL/TLS certificate. |
| <br> `ANSIBLE_GALAXY_CLI_ROLE_OPTS` | <br>  Passes any flags, such as `-no-deps`, to the role installation. |

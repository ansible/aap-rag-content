...
class _Chunk:
    """Simple chunk representation without llama_index dependencies."""
...
"""
  ...
  - embedding_dimension: {dimension}
    ...
    def _split_text(self, text: str) -> list[str]:
        ...
        Uses a simple character-based splitting approach.
        """
        chunks = []
        ...
        while start < text_len:
            ...
            if chunk and self._got_whitespace(chunk):
                chunks.append(chunk)
            ...
            if chunk_size <= chunk_overlap and start == 0:
                start = chunk_size

        return chunks

    def _split_and_filter(self, docs: list[Any]) -> list[_Chunk]:
        """Split documents into chunks and filter out invalid ones."""
        import uuid
        ...
        for doc in docs:
            ...
            for chunk_text in text_chunks:
                if self._got_whitespace(chunk_text):
                    chunk = _Chunk(
                        text=chunk_text,
                    ...
                    LOG.debug("Skipping chunk without whitespace: %s", repr(chunk_text))
...
class DocumentProcessor:
    ...
    def _get_db(self) -> _LlamaStackDB:
